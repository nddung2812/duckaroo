...
import { setPasswordEmail, resetPasswordEmail } from "./templates";

const DEFAULT_FROM = "Duckaroo <accounts@duckaroo.com.au>";

/** Canonical site origin, used to build the links inside auth emails. */
export function appUrl() {
  const configured = process.env.APP_URL?.trim();
  if (configured) return configured.replace(/\/+$/, "");
  return "https://duckaroo.com.au";
}
...
export function sendSetPasswordEmail({ to, firstName, token }) {
  return sendEmail({ to, ...setPasswordEmail({ firstName, url: setPasswordUrl(token) }) });
}

/** Ordinary forgot-password reset. */
export function sendResetPasswordEmail({ to, firstName, token }) {
  return sendEmail({ to, ...resetPasswordEmail({ firstName, url: resetPasswordUrl(token) }) });
}
